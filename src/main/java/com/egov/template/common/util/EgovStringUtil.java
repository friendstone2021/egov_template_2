...
public class EgovStringUtil {
     ...
     * @see
     */
    public static String getEncdDcd(String srcString, String srcCharsetNm, String cnvrCharsetNm) throws Exception {
    ...
    /**
     * 알림 제목 및 내용 키워드 변경 처리
     * @param texts
     * @param jobParam
     */
    public static String replaceNtcnTexts(String texts, Map<String, Object> jobParam) {
        if (texts != null && jobParam != null) {
            for (String key : jobParam.keySet()) {
                if (jobParam.get(key) instanceof String
                        || jobParam.get(key) instanceof Integer
                        || jobParam.get(key) instanceof Long) {
                    texts = texts.replaceAll("@\\{" + key + "\\}", jobParam.get(key).toString());
                }
            }
        }
        return texts;
